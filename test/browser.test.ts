  ...
  const srv = http.createServer((req, res) => {
    ...
    if (url.pathname === '/api/subscribe') {
      ...
      const sheet = {
        ...
        priceEth: '0.0015',
        priceNote: 'about $5 a month — priced in ETH, so the dollar figure moves with the market',
        days: 30,
        feePct: 10,
        feeMaxEth: '0.0002',
      ...
      if (req.method === 'POST') {
        return void readBody().then((body) => {
          ...
          if (!/^0x[0-9a-fA-F]{64}$/.test(hash)) {
            return json({ error: 'That does not look like a transaction hash.' }, 400);
          }
          subscribed = true;
          json({
            ...sheet,
            status: { active: true, daysLeft: 30, until: '2026-09-21T00:00:00.000Z',
              message: 'Auto-mint is on. 30 days left, until 2026-09-21.' },
            note: 'Thanks — 0.0015 ETH received. Auto-mint is on for 30 days.',
          });
        });
      }
      return json({
        ...sheet,
        status: subscribed
          ? { active: true, daysLeft: 30, until: '2026-09-21T00:00:00.000Z',
              message: 'Auto-mint is on. 30 days left, until 2026-09-21.' }
          : { active: false,
              message: 'Auto-mint needs a subscription: 0.0015 ETH for 30 days.' },
      });
    }
...
describe.skipIf(!chromiumPath)('the app in a browser', () => {
  ...
  it('keeps the ten addresses out of the way until asked for', async () => {
    ...
    expect(await page.locator('#walletList .row.static:visible').count()).toBe(0);

    await page.click('summary:has-text("Wallet addresses")');
    await page.waitForSelector('#walletList .row.static:visible', { timeout: 8000 });
    expect(await page.locator('#walletList .row.static:visible').count()).toBe(10);
    expect(errors).toEqual([]);
    await page.close();
  });
...
/**
 * What the screen says about money.
 *
 * These wallets were generated by this app and their keys are held by it, so a
 * charge the screen does not mention is a charge the person paying has no way
 * to find. The assertions below are about wording as much as behaviour: the
 * price, the address, and the sentence distinguishing this app's fee from the
 * network's. Anything that quietly drops one of those should fail here.
 */
describe('the charges, on screen', () => {
  it('says what auto-mint costs, where to pay, and what stays free', async () => {
    const { page, errors } = await openPage();
    await page.click('#navAccount');
    await page.waitForSelector('#payHash', { timeout: 8000 });

    const text = (await page.textContent('#app')) ?? '';
    expect(text).toContain('0.0015');
    expect(text).toContain('0x643548aB552dfD4E3B402d03c5723a12FcEFa446');
    expect(text).toMatch(/30 days/);
    expect(text).toMatch(/scanning, the live board/i);
    expect(errors).toEqual([]);
    await page.close();
  });

  it('calls the per-mint fee a charge by this app, not a network cost', async () => {
    const { page, errors } = await openPage();
    await page.click('#navAccount');
    await page.waitForSelector('#payHash', { timeout: 8000 });
    ...
    expect(text).toMatch(/of that mint's own gas/);
    expect(text).toMatch(/charge by this app, not a network cost/i);
    expect(errors).toEqual([]);
    await page.close();
  });

  it('unlocks auto-mint once a payment checks out', async () => {
    const { page, errors } = await openPage();
    await page.click('#navAccount');
    await page.waitForSelector('#payHash', { timeout: 8000 });

    await page.fill('#payHash', `0x${'ab'.repeat(32)}`);
    await page.click('button:has-text("Unlock auto-mint")');
    await page.waitForSelector('text=/days? left/', { timeout: 8000 });

    expect(await page.textContent('#app')).toMatch(/30\s*days? left/);
    expect(errors).toEqual([]);
    await page.close();
  });

  it('explains a rejected hash instead of failing silently', async () => {
    const { page, errors } = await openPage();
    await page.click('#navAccount');
    await page.waitForSelector('#payHash', { timeout: 8000 });

    await page.fill('#payHash', 'not-a-hash');
    await page.click('button:has-text("Unlock auto-mint")');
    await page.waitForSelector('.msg.err', { timeout: 8000 });

    expect(await page.textContent('.msg.err')).toMatch(/transaction hash/i);
    // The 400 itself is the point of this test, so the console error it logs
  ...
  it('warns when auto-mint is switched on but unpaid', async () => {
    // The switch flips and nothing happens otherwise, which reads as a bug.
    const { page, errors } = await openPage();
    await page.click('#navAccount');
    await page.waitForSelector('#payHash', { timeout: 8000 });

    expect(await page.textContent('#app')).toMatch(/until the subscription below is paid/);
    expect(errors).toEqual([]);
    await page.close();
  });
});
